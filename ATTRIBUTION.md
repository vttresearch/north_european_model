# Where this repository's data comes from

The model is assembled from several outside data sources. This file records what each
one is, what was done to it, and under what terms it is redistributed -- the credit that
CC BY and similar licences require be given. The repository's own licence is in
[LICENSE](LICENSE).

A section marked *not yet documented* is a gap, not an absence: the data is in the
repository or the build instructions tell you to download it, and nobody has yet written
down where it came from. Later passes fill these in.


## Map assets

`tools/maps/country_shapes.geojson` and `tools/maps/zone_shapes.geojson`, the two maps
`tools/input_data_summary.py` draws.
...
repository's licence, **CC BY 4.0** -- the same licence Mopo carries.

## Electricity demand profiles: ENTSO-E TYNDP 2024

*Not yet documented.* Downloaded by whoever runs a build, following the instructions in
[README.md](README.md), and not redistributed here. Not covered by this repository's
licence; ENTSO-E's terms apply.


## Wind and solar capacity factors: Copernicus PECD

*Not yet documented.* Downloaded by whoever runs a build, following the instructions in
[README.md](README.md), and not redistributed here. Not covered by this repository's
licence; the Copernicus terms apply.


## Hydro, district heating, hydrogen and industry timeseries

*Not yet documented.* These ship in `src_files/timeseries/` and in the source workbooks
under `src_files/data_files/`. Where each came from, and what its source terms say about
redistribution, still needs recording.
