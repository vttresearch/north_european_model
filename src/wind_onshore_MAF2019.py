...
class wind_onshore_MAF2019:
    """
    Class to process wind onshore capacity factor timeseries for countries.
    ...
    def run(self):
        ...
        input_file = os.path.join(self.input_folder, "PECD-MAF2019-wide-WindOnshore.csv")
        print(f"   Reading wind onshore cf data from '{input_file}'.. ")
        if not os.path.isfile(input_file):
            print(f"Warning! Input file '{input_file}' not found, skipping wind onshore time series!!!")
            return
...
if __name__ == '__main__':
    ...
    output_file = os.path.join(output_folder, 'test_output_wind_onshore_cf.csv')
    country_codes = [
        'FI00', 'FR00', 'NOM1'
    ]
    ...
    processor = wind_onshore_MAF2019(input_folder, country_codes, start_date, end_date)
