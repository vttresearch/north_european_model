...
class VRE_PECD:
    ...
    def run(self):
        ...
        if not os.path.isdir(self.csv_folder):
            print(f"Warning: The folder {self.csv_folder} does not exist.")
            return None

        # Check that the folder contains at least one CSV file
        csv_files = glob.glob(os.path.join(self.csv_folder, "*.csv"))
        if not csv_files:
            print(f"Warning: No CSV files found in {self.csv_folder}.")
            return None

        print(f"  Processing input files in {self.csv_folder}...")
    ...
    def _read_and_process_csv(self, file, country_code_mapping, master_index):
        ...
        if 'Date' not in df_csv.columns:
            print(f"File {file} does not have a 'Date' column. Skipping the file.")
            return None
    ...
    def _read_and_compile_input_CSVs(self, csv_folder, country_codes, start_date, end_date):
        ...
        csv_files = glob.glob(os.path.join(csv_folder, "*.csv"))
        print(f"    Using {len(filtered_files)} files within date range from the found {len(csv_files)} files.")
        ...
        try:
            ...
            print(f"Error reading the first file {first_file} for mapping: {e}")
            return df_csv_summary

        if 'Date' not in df_first.columns:
            print(f"File {first_file} does not have a 'Date' column. Cannot determine country code mapping.")
            return df_csv_summary

        country_code_mapping = self._process_country_code_mapping(df_first.columns, country_codes)

        if not country_code_mapping:
            print("No country code mappings found.")
        ...
        if alternative_mappings:
            print("    Alternative country code mappings used:")
            for country_code, col in alternative_mappings.items():
                print(f"      {country_code}: {col}")
