...
class VRE_MAF2019:
    ...
    def run_processor(self):
        ...
        log_status(f"Reading cf data from '{file_path}'.. ", self.processor_log)
        if not os.path.isfile(file_path):
            log_status(f"Warning! Input file '{file_path}' not found, skipping time series!!!", self.processor_log, level="warn")
            return None
        
        log_status(f"Processing the input file.. ", self.processor_log)
